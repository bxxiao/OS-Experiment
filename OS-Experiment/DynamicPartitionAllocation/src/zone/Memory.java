...
public class Memory {
     ...
     */
    public Zone requestInModel(int reqSize, int model){
        //使用ff算法找出适合的空闲分区
        Zone zone = null;
        if (model==FF){
            zone = searchInFF(reqSize);
        }
        if (model==BF){
    ...
    private Zone allocate(Zone zone, int reqSize){
        ...
        //否则切割当前结点（zone）
            ...
            Zone newZone = new Zone(zone.getStart(), reqSize);
            newZone.setLeisure(false);
    ...
    /**
     * 使用首次适应算法（FF）查找符合要求的空闲分区
     * 返回找到的空闲分区结点；若找不到，返回null
     */
    private Zone searchInFF(int reqSize){
        Zone current = zoneList;
        while(current!=null){
            if(current.isLeisure() && current.getSize()>reqSize){
                return current;
            }
     ...
     * 返回找到的空闲分区结点；若找不到，返回null
     */
    private Zone searchInBF(int reqSize){
    ...
    public void printZoneList(){
        ...
        int index = 1;
        System.out.println("----------------------------------------------");
        System.out.println("分区结点号    始址     分区大小    状态");
        while(current!=null){
    ...
    private void sortBfZones(){
        ...
        Collections.sort(bfZones, Comparator.comparingInt(Zone::getSize));//升序排序
    }
}
